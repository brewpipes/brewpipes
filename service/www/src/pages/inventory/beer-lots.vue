        ...
        <v-row align="stretch">
          <v-col cols="12" md="7">
            <v-card class="sub-card" variant="outlined">
              <v-card-title>Beer lot list</v-card-title>
              <v-card-text>
                <v-alert
                  v-if="errorMessage"
                  class="mb-3"
                ...
                <v-table class="data-table" density="compact">
                  ...
                  <tbody>
                    ...
                    <tr v-if="beerLots.length === 0">
                      <td colspan="3">No beer lots yet.</td>
                    </tr>
                  </tbody>
                </v-table>
              </v-card-text>
          ...
          <v-col cols="12" md="5">
            <v-card class="sub-card" variant="tonal">
              <v-card-title>Create beer lot</v-card-title>
              <v-card-text>
...
async function loadBeerLots() {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'Unable to load beer lots'
...
async function createBeerLot() {
  try {
    ...
    const message = error instanceof Error ? error.message : 'Unable to create beer lot'
    errorMessage.value = message
